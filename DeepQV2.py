    ...
    def _build_model(self):
        ...
        if(EMBTOGGLE == 0):
            model.add(Dense(64, input_shape=(
                self.state_size,), activation='tanh'))
        else:
            model.add(Dense(64, input_shape=(2*DIMENSION,), activation='tanh'))
...
class Environment:
    def __init__(self, row_x, col_y, args):
        ...
        self.cs = 0
        emb_path1 = EMBEDPATH + str(args.embedpath1)
        emb_path2 = EMBEDPATH + str(args.embedpath2)
        self.model1 = KeyedVectors.load_word2vec_format(
            emb_path1, binary=False)
        self.model2 = KeyedVectors.load_word2vec_format(
            emb_path2, binary=False)
    ...
    def generate_embedding(self):
        state = self.cs
        embed = np.zeros(2*self.dimension)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ep1", "--embedpath1", help="Path for embeddings src")
    parser.add_argument("-ep2", "--embedpath2", help="Path for embeddings dst")
